...
public class TransferService {
  ...
  public TransferResultDto transfer(Long fromAccountId, Long toAccountId, BigDecimal amount) {
    ...
    Account fromAccount =
        accountRepository
            .findByIdToUpdate(fromAccountId)
            .filter(Account::getActive)
            .orElseThrow(RuntimeException::new);

    Account toAccount =
        accountRepository
            .findByIdToUpdate(toAccountId)
            .filter(Account::getActive)
    ...
    MoneyTransaction moneyTransaction =
        transferService.saveMoneyTransaction(
    ...
    return new TransferResultDto(
        String.valueOf(moneyTransaction.getId()),
        TransferStatus.SUCCESS,
        moneyTransaction.getTimeStamp());
  }
